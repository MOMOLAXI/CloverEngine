...
namespace Clover
{
    ...
    internal class XorManaged : SymmetricAlgorithm
    {
        ...
        {
            byte[] passwordArray = CryptoHelpers.Combine(new byte[] { (byte)passwordBytes.Length }, passwordBytes);
            ushort verifier = 0x0000;
            for (int i = 0; i < passwordArray.Length; ++i)
            {
                byte passwordByte = passwordArray[passwordArray.Length - 1 - i];
        ...
        internal static ushort CreateXorKey_Method1(byte[] passwordBytes)
        {
            ...
            {
                byte c = passwordBytes[passwordBytes.Length - 1 - i];
                for (int j = 0; j < 7; ++j)
                {
                    if ((c & 0x40) != 0)
                    {
        ...
        internal static byte[] CreateXorArray_Method1(byte[] passwordBytes)
        {
            ...
            ushort xorKey = CreateXorKey_Method1(passwordBytes);
