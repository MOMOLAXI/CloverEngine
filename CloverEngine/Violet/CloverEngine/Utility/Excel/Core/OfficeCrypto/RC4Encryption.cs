...
namespace Clover
{
    ...
    internal class RC4Encryption : EncryptionInfo
    {
        ...
        public static byte[] GenerateSecretKey(string password, byte[] salt)
        {
            ...
            byte[] h = CryptoHelpers.HashBytes(System.Text.Encoding.Unicode.GetBytes(password), HashIdentifier.MD5);
        ...
        public override bool VerifyPassword(string password)
        {
            ...
            using (SymmetricAlgorithm cipher = CryptoHelpers.CreateCipher(CipherIdentifier.RC4, 0, 0, 0))
            {
                using (ICryptoTransform transform = cipher.CreateDecryptor(blockKey, null))
                {
                    byte[] decryptedVerifier = CryptoHelpers.DecryptBytes(transform, EncryptedVerifier);
                    byte[] decryptedVerifierHash = CryptoHelpers.DecryptBytes(transform, EncryptedVerifierHash);

                    byte[] verifierHash = CryptoHelpers.HashBytes(decryptedVerifier, HashIdentifier.MD5);
                    for (int i = 0; i < 16; ++i)
                    {
