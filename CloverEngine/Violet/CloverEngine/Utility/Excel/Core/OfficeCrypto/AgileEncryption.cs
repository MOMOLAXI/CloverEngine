...
namespace Clover
{
    ...
    internal class AgileEncryption : EncryptionInfo
    {
        ...
        {
            using (MemoryStream stream = new MemoryStream(bytes, 8, bytes.Length - 8))
            {
        ...
        {
            using (SymmetricAlgorithm cipher = CryptoHelpers.CreateCipher(PasswordCipherAlgorithm, PasswordKeyBits, PasswordBlockSize * 8, PasswordCipherChaining))
            {
        ...
        {
            byte[] salt = CryptoHelpers.HashBytes(CryptoHelpers.Combine(secretKey, BitConverter.GetBytes(blockNumber)), HashAlgorithm);
        ...
        {
            byte[] secretKey;
            using (HashAlgorithm hashAlgorithm = CryptoHelpers.Create(PasswordHashAlgorithm))
                secretKey = HashPassword(password, PasswordSaltValue, hashAlgorithm, PasswordSpinCount);

            byte[] inputBlockKey = CryptoHelpers.HashBytes(
            ...
            byte[] valueBlockKey = CryptoHelpers.HashBytes(
            ...
            using (SymmetricAlgorithm cipher = CryptoHelpers.CreateCipher(PasswordCipherAlgorithm, PasswordKeyBits, PasswordBlockSize * 8, PasswordCipherChaining))
            {
                byte[] decryptedVerifier = CryptoHelpers.DecryptBytes(cipher, PasswordEncryptedVerifierHashInput, inputBlockKey, PasswordSaltValue);
                byte[] decryptedVerifierHash = CryptoHelpers.DecryptBytes(cipher, PasswordEncryptedVerifierHashValue, valueBlockKey, PasswordSaltValue);

                byte[] verifierHash = CryptoHelpers.HashBytes(decryptedVerifier, PasswordHashAlgorithm);
        ...
        {
            byte[] block3 = new byte[] { 0x14, 0x6e, 0x0b, 0xe7, 0xab, 0xac, 0xd0, 0xd6 };

            byte[] hash;
            using (HashAlgorithm hashAlgorithm = CryptoHelpers.Create(hashIdentifier))
            {
            ...
            for (int i = hashSize; i < keyBits / 8; i++)
            {
                hash[i] = 0x36;
            }

            // NOTE: the stored salt is padded to a multiple of the block size which affects AES-192
            byte[] decryptedKeyValue = CryptoHelpers.DecryptBytes(cipher, encryptedKeyValue, hash, saltValue);
        ...
        {
            byte[] h = hashAlgorithm.ComputeHash(CryptoHelpers.Combine(saltValue, System.Text.Encoding.Unicode.GetBytes(password)));
