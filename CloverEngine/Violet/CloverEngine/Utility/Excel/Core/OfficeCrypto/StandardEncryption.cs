...
namespace Clover
{
    ...
    internal class StandardEncryption : EncryptionInfo
    {
        ...
        public StandardEncryption(byte[] bytes)
        {
            ...
            StandardCipher cipher = (StandardCipher)BitConverter.ToUInt32(bytes, 20);

            StandardHash hashAlgorithm = (StandardHash)BitConverter.ToUInt32(bytes, 24);
            ...
            StandardProvider providerType = (StandardProvider)BitConverter.ToUInt32(bytes, 32);
            ...
            int saltSize = BitConverter.ToInt32(bytes, 12 + headerSize);
            ...
            int verifierHashSize = ((Flags & EncryptionHeaderFlags.AES) != 0) ? 32 : 20;
        ...
        public override bool VerifyPassword(string password)
        {
            ...
            byte[] blockKey = ((Flags & EncryptionHeaderFlags.AES) != 0) ? secretKey : GenerateBlockKey(0, secretKey);

            using (SymmetricAlgorithm cipher = CryptoHelpers.CreateCipher(CipherAlgorithm, KeySize, BlockSize, CipherMode.ECB))
            {
                using (ICryptoTransform transform = cipher.CreateDecryptor(blockKey, SaltValue))
                {
                    byte[] decryptedVerifier = CryptoHelpers.DecryptBytes(transform, Verifier);
                    byte[] decryptedVerifierHash = CryptoHelpers.DecryptBytes(transform, VerifierHash);

                    byte[] verifierHash = CryptoHelpers.HashBytes(decryptedVerifier, HashAlgorithm);
                    for (int i = 0; i < 16; ++i)
                    {
