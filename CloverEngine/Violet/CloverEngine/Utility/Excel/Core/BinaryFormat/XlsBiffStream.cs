...
namespace Clover
{
    ...
    internal class XlsBiffStream : IDisposable
    {
        public XlsBiffStream(Stream baseStream, int offset = 0, int explicitVersion = 0, string password = null, byte[] secretKey = null, EncryptionInfo encryption = null)
        {
            ...
            {
                if (!(Read() is XlsBiffFilePass filePass))
        ...
        public XlsBiffRecord Read()
        {
            ...
            XlsBiffRecord record = GetRecord(BaseStream);
        ...
        {
            int recordOffset = (int)stream.Position;
            byte[] header = new byte[4];
            stream.Read(header, 0, 4);

            // Does this work on a big endian system?
            BIFFRECORDTYPE id = (BIFFRECORDTYPE)BitConverter.ToUInt16(header, 0);
            int recordSize = BitConverter.ToUInt16(header, 2);

            byte[] bytes = new byte[4 + recordSize];
        ...
        {
            byte[] bytes = new byte[blockOffset];
        ...
        private void DecryptRecord(int startPosition, BIFFRECORDTYPE id, byte[] bytes)
        {
            ...
            {
                int offset = startPosition + position;
                ...
                if (Encryption.IsXor)
                {
                    ...
                    XorManaged.XorTransform xorTransform = (XorManaged.XorTransform)CipherTransform;
                    xorTransform.XorArrayIndex = offset + recordSize - 4;
                }

                // Decrypt at most up to the next 1024 byte boundary
                int chunkSize = (int)Math.Min(recordSize - position, 1024 - blockOffset);
                byte[] block = new byte[chunkSize];

                Array.Copy(bytes, position, block, 0, chunkSize);

                byte[] decryptedblock = CryptoHelpers.DecryptBytes(CipherTransform, block);
