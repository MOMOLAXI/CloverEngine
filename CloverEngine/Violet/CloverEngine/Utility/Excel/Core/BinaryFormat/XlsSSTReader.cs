...
namespace Clover
{
    ...
    internal class XlsSSTReader
    {
        ...
        public IEnumerable<IXlsString> ReadStringsFromSST(XlsBiffSST sst)
        {
            ...
            while (true)
            {
                if (!TryReadString(out IXlsString result))
                {
                    yield break;
                }
        ...
        private bool TryReadString(out IXlsString result)
        {
            ...
            if (CurrentState == SstState.StringData)
            {
                int bytesPerCharacter = CurrentIsMultiByte ? 2 : 1;
                int maxRecordCharacters = (CurrentRecord.Size - CurrentRecordOffset) / bytesPerCharacter;
                int readCharacters = Math.Min(maxRecordCharacters, CurrentRemainingCharacters);
            ...
            if (CurrentState == SstState.StringTail)
            {
                ...
                if (!Advance(CurrentTailBytes, out int advanceBytes))
                {
                    result = null;
        ...
        private byte ReadByte()
        {
            ...
            byte result = CurrentRecord.Bytes[CurrentRecordOffset];
            CurrentRecordOffset++;
            return result;
        }
