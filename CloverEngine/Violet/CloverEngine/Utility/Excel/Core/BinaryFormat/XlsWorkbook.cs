...
namespace Clover
{
    ...
    internal class XlsWorkbook : CommonWorkbook, IWorkbook<XlsWorksheet>
    {
        internal XlsWorkbook(Stream stream, string password, Encoding fallbackEncoding)
        {
            ...
            using (XlsBiffStream biffStream = new XlsBiffStream(stream, 0, 0, password))
            {
        ...
        {
            Dictionary<int, XlsBiffFormatString> formats = new Dictionary<int, XlsBiffFormatString>();
            ...
            foreach (KeyValuePair<int, XlsBiffFormatString> format in formats)
            {
