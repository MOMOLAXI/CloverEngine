...
namespace Clover
{
    ...
    internal class XlsWorksheet : IWorksheet
    {
        ...
        */
        ...
        {
            int rowIndex = 0;
            using (XlsBiffStream biffStream = new XlsBiffStream(Stream, (int) DataOffset, Workbook.BiffVersion, null,
                Workbook.SecretKey, Workbook.Encryption))
            {
                foreach (Row rowBlock in ReadWorksheetRows(biffStream))
                {
        ...
        private void GetBlockSize(int startRow, out int blockRowCount, out int minOffset, out int maxOffset)
        {
            ...
            {
                if (RowOffsetMap.TryGetValue(startRow + i, out XlsRowOffset rowOffset))
                {
                    ...
                    {
                        int maxOverlapRowCount = rowOffset.MaxOverlapRowIndex + 1;
        ...
        private IEnumerable<Row> ReadWorksheetRows(XlsBiffStream biffStream)
        {
            ...
            {
                GetBlockSize(rowIndex, out int blockRowCount, out int minOffset, out int maxOffset);

                XlsRowBlock block = ReadNextBlock(biffStream, rowIndex, blockRowCount, minOffset, maxOffset);

                for (int i = 0; i < blockRowCount; ++i)
                {
                    if (block.Rows.TryGetValue(rowIndex + i, out Row row))
                    {
                        yield return row;
                    }
                }
        ...
        private XlsRowBlock ReadNextBlock(XlsBiffStream biffStream, int startRow, int rows, int minOffset,
            int maxOffset)
        {
            XlsRowBlock result = new XlsRowBlock {Rows = new Dictionary<int, Row>()};
            ...
            while (biffStream.Position <= maxOffset && (rec = biffStream.Read()) != null)
            {
                ...
                {
                    Row currentRow = EnsureRow(result, cell.RowIndex);

                    if (cell.Id == BIFFRECORDTYPE.MULRK)
                    {
                        IEnumerable<Cell> cellValues = ReadMultiCell(cell);
                        currentRow.Cells.AddRange(cellValues);
                    }
                    else
                    {
                        int xfIndex = GetXfIndexForCell(cell, ixfe);
                        Cell cellValue = ReadSingleCell(biffStream, cell, xfIndex);
        ...
        {
            if (!result.Rows.TryGetValue(rowIndex, out Row currentRow))
            {
                double height = DefaultRowHeight / 20.0;
                if (RowOffsetMap.TryGetValue(rowIndex, out XlsRowOffset rowOffset) && rowOffset.Record != null)
                {
                    height = (rowOffset.Record.UseDefaultRowHeight ? DefaultRowHeight : rowOffset.Record.RowHeight) /
                             20.0;
                }
        ...
        private IEnumerable<Cell> ReadMultiCell(XlsBiffBlankCell cell)
        {
            ...
            switch (cell.Id)
            {
                case BIFFRECORDTYPE.MULRK:
                    ...
                    {
                        ushort xfIndex = rkCell.GetXF(j);
                        ExtendedFormat effectiveStyle = Workbook.GetEffectiveCellStyle(xfIndex, cell.Format);

                        object value = TryConvertOADateTime(rkCell.GetValue(j), effectiveStyle.NumberFormatIndex);
        ...
        private Cell ReadSingleCell(XlsBiffStream biffStream, XlsBiffBlankCell cell, int xfIndex)
        {
            ...
            ExtendedFormat effectiveStyle = Workbook.GetEffectiveCellStyle(xfIndex, cell.Format);
            int numberFormatIndex = effectiveStyle.NumberFormatIndex;
        ...
        private string GetLabelString(XlsBiffLabelCell cell, ExtendedFormat effectiveStyle)
        {
            ...
            Encoding labelEncoding = GetFont(effectiveStyle.FontIndex)?.ByteStringEncoding ?? Encoding;
        ...
        private object TryGetFormulaValue(XlsBiffStream biffStream, XlsBiffFormulaCell formulaCell,
            ExtendedFormat effectiveStyle, out CellError? error)
        {
            error = null;
        ...
        private string TryGetFormulaString(XlsBiffStream biffStream, ExtendedFormat effectiveStyle)
        {
            ...
            {
                XlsBiffFormulaString stringRecord = (XlsBiffFormulaString) rec;
        ...
        {
            NumberFormatString format = Workbook.GetNumberFormatString(numberFormatIndex);
            if (format != null)
            {
        ...
        private int GetXfIndexForCell(XlsBiffBlankCell cell, XlsBiffRecord ixfe)
        {
            if (Workbook.BiffVersion == 2)
            {
                ...
                {
                    ushort xFormat = ixfe.ReadUInt16(0);
                    return xFormat;
                }
        ...
        private void ReadWorksheetGlobals()
        {
                ...
                Workbook.SecretKey, Workbook.Encryption))
            {
                ...
                List<CellRange> mergeCells = new List<CellRange>();
                Dictionary<ushort, XlsBiffFormatString> biffFormats = new Dictionary<ushort, XlsBiffFormatString>();
                int recordOffset = biffStream.Position;
                XlsBiffRecord rec = biffStream.Read();
                ...
                while (rec != null && !(rec is XlsBiffEof))
                {
                    switch (rec)
                    {
                        ...
                        case XlsBiffSimpleValueRecord is1904 when rec.Id == BIFFRECORDTYPE.RECORD1904:
                            ...
                            break;
                        case XlsBiffXF xf when rec.Id == BIFFRECORDTYPE.XF_V2 || rec.Id == BIFFRECORDTYPE.XF_V3 ||
                                               rec.Id == BIFFRECORDTYPE.XF_V4:
                        ...
                        case XlsBiffSimpleValueRecord codePage when rec.Id == BIFFRECORDTYPE.CODEPAGE:
                            Encoding = Helper.GetEncoding(codePage.Value);
                            break;
                        ...
                        case XlsBiffBlankCell cell:
                            ...
                            break;
                        case { } when rec.Id == BIFFRECORDTYPE.IXFE:
                ...
                foreach (KeyValuePair<ushort, XlsBiffFormatString> biffFormat in biffFormats)
                {
        ...
        {
            if (!RowOffsetMap.TryGetValue(rowIndex, out XlsRowOffset rowOffset))
            {
