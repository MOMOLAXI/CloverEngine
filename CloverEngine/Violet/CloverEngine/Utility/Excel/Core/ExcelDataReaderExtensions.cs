    ...
    public static class ExcelDataReaderExtensions
    {
        ...
        public static DataSet AsDataSet(this IExcelDataReader self, ExcelDataSetConfiguration configuration = null)
        {
            ...
            do
            {
                ...
                ExcelDataTableConfiguration tableConfiguration = configuration.ConfigureDataTable != null
                ...
                DataTable table = AsDataTable(self, tableConfiguration);
        ...
        {
            DataTable result = new DataTable { TableName = self.Name };
            ...
            while (self.Read())
            {
                if (first)
                {
                    ...
                    for (int i = 0; i < self.FieldCount; i++)
                    {
                        ...
                        string name = configuration.UseHeaderRow
                        ...
                        DataColumn column = new DataColumn(columnName, typeof(object)) { Caption = name };
                ...
                for (int i = 0; i < columnIndices.Count; i++)
                {
                    ...
                    object value = self.GetValue(columnIndex);
                    if (configuration.TransformValue != null)
                    {
                        object transformedValue = configuration.TransformValue(self, i, value);
        ...
        private static bool IsEmptyRow(IExcelDataReader reader, ExcelDataTableConfiguration configuration)
        {
            for (int i = 0; i < reader.FieldCount; i++)
            {
                ...
                if (configuration.TransformValue != null)
                {
                    object transformedValue = configuration.TransformValue(reader, i, value);
                    if (transformedValue != null)
                        value = transformedValue;
                }
