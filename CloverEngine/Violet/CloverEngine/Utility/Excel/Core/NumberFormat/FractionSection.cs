...
namespace Clover
{
    internal class FractionSection
    {
        ...
        public static bool TryParse(List<string> tokens, out FractionSection format)
        {
            ...
            GetNumerator(numeratorParts, out List<string> integerPart, out List<string> numeratorPart);

            if (!TryGetDenominator(denominatorParts, out List<string> denominatorPrefix, out List<string> denominatorPart, out int denominatorConstant, out List<string> denominatorSuffix, out List<string> fractionSuffix))
            {
                format = null;
                return false;
            }
        ...
        private static bool TryGetDenominator(List<string> tokens, out List<string> denominatorPrefix, out List<string> denominatorPart, out int denominatorConstant, out List<string> denominatorSuffix, out List<string> fractionSuffix)
        {
            ...
            {
                string token = tokens[fractionSuffixIndex - 1];
                if (Token.IsPlaceholder(token))
                {
                    break;
                }
