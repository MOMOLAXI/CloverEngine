...
namespace Clover
{
    internal partial class ZipWorker : IDisposable
    {
        ...
        public ZipWorker(Stream fileStream)
        {
            ...
            foreach (ZipArchiveEntry entry in _zipFile.Entries)
            {
        ...
        {
            ZipArchiveEntry entry = FindEntry(string.Format(FileSharedStrings, Format));
            if (entry != null)
        ...
        {
            ZipArchiveEntry entry = FindEntry(string.Format(FileStyles, Format));
            if (entry != null)
        ...
        {
            ZipArchiveEntry entry = FindEntry(string.Format(FileWorkbook, Format));
            if (entry != null)
            ...
            throw new HeaderException(Errors.ErrorZipNoOpenXml);
        }
        ...
        public RecordReader GetWorksheetReader(string sheetPath)
        {
            ...
            ZipArchiveEntry zipEntry = FindEntry(sheetPath);
            if (zipEntry != null)
            {
        ...
        {
            ZipArchiveEntry zipEntry = FindEntry(string.Format(FileRels, Format));
            if (zipEntry != null)
        ...
        {
            if (_entries.TryGetValue(name, out ZipArchiveEntry entry))
                return entry;
            return null;
        }
    }
