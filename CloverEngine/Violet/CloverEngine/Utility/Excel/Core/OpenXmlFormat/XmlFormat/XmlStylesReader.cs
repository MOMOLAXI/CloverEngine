    ...
    internal sealed class XmlStylesReader : XmlRecordReader
    {
        ...
        protected override IEnumerable<Record> ReadOverride()
        {
            ...
            while (!Reader.EOF)
            {
                ...
                {
                    foreach (ExtendedFormat xf in ReadCellXfs())
                ...
                else if (Reader.IsStartElement(ElementNumberFormats, NsSpreadsheetMl))
                {
                    ...
                    while (!Reader.EOF)
                    {
                        ...
                        {
                            int.TryParse(Reader.GetAttribute(ANumFmtId), out int numFmtId);
        ...
        private IEnumerable<ExtendedFormat> ReadCellXfs()
        {
            ...
            while (!Reader.EOF)
            {
                ...
                {
                    int.TryParse(Reader.GetAttribute(AXFId), out int xfId);
                    int.TryParse(Reader.GetAttribute(ANumFmtId), out int numFmtId);
                    bool applyNumberFormat = Reader.GetAttribute(AApplyNumberFormat) == "1";
                    bool applyAlignment = Reader.GetAttribute(AApplyAlignment) == "1";
                    bool applyProtection = Reader.GetAttribute(AApplyProtection) == "1";
                    ReadAlignment(Reader, out int indentLevel, out HorizontalAlignment horizontalAlignment, out bool hidden, out bool locked);
